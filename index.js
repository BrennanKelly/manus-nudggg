...
var userProfiles = mysqlTable("user_profiles", {
  ...
  preferredNudgeTimes: json("preferredNudgeTimes").$type(),
...
var habitLogs = mysqlTable("habit_logs", {
  id: int("id").autoincrement().primaryKey(),
...
var journals = mysqlTable("journals", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
...
var journalAI = mysqlTable("journal_ai", {
  id: int("id").autoincrement().primaryKey(),
...
var nudgeSettings = mysqlTable("nudge_settings", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }).unique(),
  enabled: boolean("enabled").default(true).notNull(),
  cadence: json("cadence").$type(),
  preferredTimes: json("preferredTimes").$type(),
  style: varchar("style", { length: 50 }).default("supportive"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull()
});
var notificationLog = mysqlTable("notification_log", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  ...
  actionTaken: json("actionTaken").$type()
});
var dailyCheckIns = mysqlTable("daily_check_ins", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
...
var streakMilestones = mysqlTable("streak_milestones", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
...
async function getHabitLogs(habitId, userId, limit = 30) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(habitLogs).where(and(eq(habitLogs.habitId, habitId), eq(habitLogs.userId, userId))).orderBy(desc(habitLogs.completedAt)).limit(limit);
}
async function logHabitCompletion(log) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(habitLogs).values(log);
}
async function getHabitStreak(habitId, userId) {
  const db = await getDb();
  if (!db) return 0;
  const logs = await db.select().from(habitLogs).where(and(eq(habitLogs.habitId, habitId), eq(habitLogs.userId, userId))).orderBy(desc(habitLogs.completedAt));
...
async function getUserJournals(userId, limit = 30) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(journals).where(eq(journals.userId, userId)).orderBy(desc(journals.createdAt)).limit(limit);
}
async function getJournalById(id, userId) {
  const db = await getDb();
  if (!db) return void 0;
  const result = await db.select().from(journals).where(and(eq(journals.id, id), eq(journals.userId, userId))).limit(1);
  return result.length > 0 ? result[0] : void 0;
}
...
async function updateJournal(id, userId, updates) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
async function getJournalAI(journalId, userId) {
  const db = await getDb();
  if (!db) return void 0;
  const result = await db.select().from(journalAI).where(and(eq(journalAI.journalId, journalId), eq(journalAI.userId, userId))).limit(1);
  return result.length > 0 ? result[0] : void 0;
}
async function createJournalAI(ai) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(journalAI).values(ai);
}
async function getNudgeSettings(userId) {
  const db = await getDb();
  if (!db) return void 0;
  const result = await db.select().from(nudgeSettings).where(eq(nudgeSettings.userId, userId)).limit(1);
  return result.length > 0 ? result[0] : void 0;
}
async function upsertNudgeSettings(settings) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
async function getUserNotifications(userId, limit = 50) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(notificationLog).where(eq(notificationLog.userId, userId)).orderBy(desc(notificationLog.sentAt)).limit(limit);
}
async function markNotificationOpened(id, userId) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(notificationLog).set({ openedAt: /* @__PURE__ */ new Date() }).where(and(eq(notificationLog.id, id), eq(notificationLog.userId, userId)));
}
...
var appRouter = router({
  ...
  profile: router({
    ...
    create: protectedProcedure.input(z2.object({
      ...
      motivationStyle: z2.enum(["supportive", "tough-love-lite", "humorous", "calm"]).optional(),
      quietHoursStart: z2.string().optional(),
      quietHoursEnd: z2.string().optional(),
      preferredNudgeTimes: z2.array(z2.string()).optional(),
      frictionPoints: z2.string().optional(),
      whyMissingSocial: z2.string().optional()
  ...
  goals: router({
    ...
    get: protectedProcedure.input(z2.object({ id: z2.number() })).query(async ({ ctx, input }) => {
      return await getGoalById(input.id, ctx.user.id);
    }),
    create: protectedProcedure.input(z2.object({
      title: z2.string().min(1),
      category: z2.string().optional(),
      whyItMatters: z2.string().optional(),
  ...
  habits: router({
    ...
    get: protectedProcedure.input(z2.object({ id: z2.number() })).query(async ({ ctx, input }) => {
      return await getHabitById(input.id, ctx.user.id);
    }),
    create: protectedProcedure.input(z2.object({
      name: z2.string().min(1),
      frequencyType: z2.enum(["daily", "weekly"]).optional(),
      frequencyCount: z2.number().optional(),
      reminderTime: z2.string().optional()
    })).mutation(async ({ ctx, input }) => {
      await createHabit({
        userId: ctx.user.id,
        ...input
    ...
    update: protectedProcedure.input(z2.object({
      ...
      await updateHabit(id, ctx.user.id, updates);
      return { success: true };
    }),
    delete: protectedProcedure.input(z2.object({ id: z2.number() })).mutation(async ({ ctx, input }) => {
    ...
    logs: protectedProcedure.input(z2.object({ habitId: z2.number() })).query(async ({ ctx, input }) => {
      return await getHabitLogs(input.habitId, ctx.user.id);
    }),
    streak: protectedProcedure.input(z2.object({ habitId: z2.number() })).query(async ({ ctx, input }) => {
      return await getHabitStreak(input.habitId, ctx.user.id);
    })
  }),
  // Journal Management
  journals: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      return await getUserJournals(ctx.user.id);
    }),
    get: protectedProcedure.input(z2.object({ id: z2.number() })).query(async ({ ctx, input }) => {
      return await getJournalById(input.id, ctx.user.id);
    }),
    create: protectedProcedure.input(z2.object({
      entryText: z2.string().min(1),
      moodScore: z2.number().optional(),
    ...
    update: protectedProcedure.input(z2.object({
      id: z2.number(),
      entryText: z2.string().optional(),
      moodScore: z2.number().optional(),
      imageUrl: z2.string().optional()
    })).mutation(async ({ ctx, input }) => {
      const { id, ...updates } = input;
      await updateJournal(id, ctx.user.id, updates);
      return { success: true };
    }),
    delete: protectedProcedure.input(z2.object({ id: z2.number() })).mutation(async ({ ctx, input }) => {
    ...
    generateReflection: protectedProcedure.input(z2.object({
      journalId: z2.number(),
      entryText: z2.string()
    })).mutation(async ({ ctx, input }) => {
    ...
    getReflection: protectedProcedure.input(z2.object({ journalId: z2.number() })).query(async ({ ctx, input }) => {
      return await getJournalAI(input.journalId, ctx.user.id);
    })
  }),
  // Nudge Settings
  nudges: router({
    getSettings: protectedProcedure.query(async ({ ctx }) => {
      return await getNudgeSettings(ctx.user.id);
    }),
    updateSettings: protectedProcedure.input(z2.object({
      enabled: z2.boolean().optional(),
      cadence: z2.object({
        daily: z2.boolean().optional(),
        weekly: z2.boolean().optional()
      }).optional(),
      preferredTimes: z2.array(z2.string()).optional(),
      style: z2.string().optional()
    })).mutation(async ({ ctx, input }) => {
      await upsertNudgeSettings({
        userId: ctx.user.id,
        ...input
      });
      return { success: true };
    ...
    markOpened: protectedProcedure.input(z2.object({ id: z2.number() })).mutation(async ({ ctx, input }) => {
      await markNotificationOpened(input.id, ctx.user.id);
      return { success: true };
    })
  })
});
